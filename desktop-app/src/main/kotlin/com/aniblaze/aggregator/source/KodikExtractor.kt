...
class KodikExtractor @Inject constructor(
    ...
    private fun parseParams(embedUrl: String, html: String): Map<String, String>? {
        // Сама страница знает, ЧТО играть: `vInfo.type/hash/id` — это конкретная серия.
        // Для эмбедов вида /season/72095/<hash>?episode=1 (так их отдаёт YummyAnime)
        // путь URL описывает СЕЗОН, и /ftor с ним отвечает 404; страница же уже
        // выбрала серию по `?episode=`. Путь — только запасной вариант.
        fun vInfo(name: String) = Regex("""vInfo\.$name\s*=\s*'([^']*)'""").find(html)?.groupValues?.get(1)
        ...
        val type = vInfo("type") ?: seg.getOrNull(0) ?: return null
        val id = vInfo("id") ?: seg.getOrNull(1) ?: return null
        val hash = vInfo("hash") ?: seg.getOrNull(2) ?: return null
        ...
        // В разметке домен лежит как `var domain`, а /ftor ждёт его в поле `d`.
        if ("d" !in p) v("domain")?.let { p["d"] = it }
        return p
    }
    ...
    private fun decodeSrc(src: String): String? = runCatching {
        ...
        val decoded = String(java.util.Base64.getDecoder().decode(rotated))
        when {
