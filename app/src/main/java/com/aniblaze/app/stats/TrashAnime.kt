 ...
 *
 * ОТЛИЧИЕ ANDROID-ВЕРСИИ: здесь пороги те же, а ДАННЫХ для них пока нет. Ни одна
 * запись в базе телефона не хранит оценку MAL: у `ContentEntity` есть только
 * `rating`, и он приходит с разных источников по разным шкалам — у Anixart это те
 * самые пять звёзд, забракованные выше, у Shikimori/TMDB/Kodik десять баллов.
 * Смешивать их в одну цифру нельзя, поэтому подставлять `rating` вместо `malScore`
 * здесь НЕЛЬЗЯ — это ровно та ошибка, от которой отказались на ПК. Пока поля
 * malScore/malVotes/malLowVotes не появятся у тайтла, [hasVerdict] везде вернёт
 * false, и шкала честно скажет, что судить не по чему.
 */
